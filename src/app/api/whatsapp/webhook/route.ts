...
/**
 * Meta's WhatsApp webhook, used only for delivery reports.
 *
 ...
 *
 * The number is send-only by the studio's choice, so incoming messages are
 * counted and dropped. Nothing is stored; reports go to the Worker log
  ...
  try {
    ...
    for (const entry of body?.entry ?? []) {
      for (const change of entry?.changes ?? []) {
        ...
        const incoming = (value.messages ?? []).length;
        if (incoming) {
          console.log(`[whatsapp] ignored ${incoming} incoming message(s); number is send-only`);
        }
      }
    }
  } catch (e) {
